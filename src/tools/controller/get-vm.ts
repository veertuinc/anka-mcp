...
export const controllerGetVmTool = defineTool({
  ...
  config: {
    ...
    description:
      "Poll the state of a controller VM instance. Only call this when controller_request_vm " +
      "returned status pending — if request_vm already returned status ready with ssh, skip get_vm. " +
      "When SSH-ready, returns host, forwarded port, username, and ssh_connect_hint. " +
      "Do not SSH until status is ready; wait ~20s after ready before connecting (startup_script " +
      "installs your public key at boot). While provisioning, returns status pending with " +
